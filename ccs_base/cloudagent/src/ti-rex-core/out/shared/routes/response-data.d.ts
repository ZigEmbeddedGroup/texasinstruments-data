 ...
 *
 */
import { PackageAuxMetadata } from '../../lib/dbBuilder/dbBuilderUtils';
import { Compiler, Kernel, PlatformAttribute, ResourceSubClass, ModuleOf, NumericPlatformAttribute, ModuleGroup, PackageSubType, FeatureType, ViewLimitation } from '../../lib/dbBuilder/dbTypes';
import { CoreType } from '../../sqldb/db/types';
import { TrunkJson } from '../../sqldb/manage';
...
export declare namespace Nodes {
    ...
    export const LIMITATION_CONSTRAINT_MAPPING: {
        readonly [K in ViewLimitation]: Nodes.ViewConstraint;
    };
    export const CONSTRAINT_PRECEDENCE: {
        readonly [K in ViewConstraint]: number;
    };
...
export interface ModuleGroupPackageMetadata {
    id: string;
    version: string;
}
interface DevPackageMetadataData {
    id: string;
    name: string;
    version: string;
    type: string;
...
export interface CommonDevMetadataData {
    id: string;
    name: string;
    type: string;
}
export type DeviceMetadataData = CommonDevMetadataData & {
    parent?: string;
    coreTypes?: CoreType[];
};
export type DevtoolMetadataData = CommonDevMetadataData & {
    devices: string[];
    connections: string[];
};
export interface BaseDevPackageMetadata {
    package: DevPackageMetadataData;
    packagePath: string;
}
export type DevicePackageMetadata = BaseDevPackageMetadata & {
    devices: DeviceMetadataData[];
};
export type DevtoolPackageMetadata = BaseDevPackageMetadata & {
    devtools: DevtoolMetadataData[];
};
export interface GlobalMetadataData {
    devicePackages: DevicePackageMetadata[];
    devtoolPackages: DevtoolPackageMetadata[];
    packageAux: PackageAuxMetadata;
    foundationTree: TrunkJson;
}
...
export interface PackageData {
    ...
    descriptionAsDependency?: string;
}
