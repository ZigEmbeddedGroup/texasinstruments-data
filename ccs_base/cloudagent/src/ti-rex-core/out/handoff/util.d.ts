...
export interface PackageEntryStaged extends PackageEntryBase {
    ...
    backupZips: string[];
    backupFolder: string;
    backupEntry: Omit<PackageEntryBase, 'id' | 'version' | 'content' | 'basePath' | 'zips'>;
}
 ...
 * @param onProgressUpdate
 *
 *  @returns  topLevelItems - where topLevelItems is the
 ...
 * @param promiseFn
 *
 * @returns result - or null if there was an error
 */
