...
export declare enum Option {
    URL = 0,// on its own, only latest versions
    SCHEMA = 1,// just schema
    REFRESH_SCHEMA = 2,// schema + refresh (data validation)
    ALL = 3
}
...
export declare const enum RefreshMessageLevel {
    NONE = 0,
    WARNING = 1,// If only warnings the refresh will be treated as a success
    ERROR_CONTINUE = 2,// Current package refresh can continue to detect more problems. Not suitable for publishing
    CRITICAL_ABORT_PACKAGE = 3,// Current package refresh stops
...
export declare function packagePresentAndValidityChecks(originalFullPackageList: RefreshRequestEntry[], logger: BaseLogger, validationType?: ValidationType): Promise<{
    allValid: boolean;
    refreshAll: boolean;
 ...
 *
 */
export declare function _refreshDatabase({ contentPackages, dbs, refreshAll, packageUids, dbBasePath, logger }: {
