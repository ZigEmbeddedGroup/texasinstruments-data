...
class FlashPython(FlashPythonInstance):
    ...
    def load_image(self, images, propertiesMap):
        ...
        for image in images:
            ...
            status = self._send_image(image.path, propertiesMap)
            ...
            if status == statuscodes[BOOTLOADER_UNIFLASH_STATUSCODE_SUCCESS]:
                ...
                self.push_message(str(status), FP_TRACE_LEVEL_FATAL)
    ...
    def _send_image(self, filepath, propertiesMap):
        try:
            ...
            app_image_extension = (".appimage",".mcelf", ".hs_fs", ".hs", ".bin")
            if (self.device in ["am263x", "am263px", "am261x"]):
                app_image_extension = (".mcelf", ".hs_fs", ".hs", ".bin")
            hsmrtimage_extension = ".hsmimage.bin"
            custom_offset = int(propertiesMap['FlashAppOffset'], 16)
            flash_sector_size = int(propertiesMap['FlashSectorSize'], 16)
            if filepath.endswith(app_image_extension) and (not filepath.endswith(hsmrtimage_extension)):
                if (custom_offset == 0) or (custom_offset % flash_sector_size != 0):
                    self.push_message(str("Invaild application image offset. Please make sure the application image offset is not zero and aligned with 128KB"), FP_TRACE_LEVEL_FATAL)
                    return
                offset = custom_offset

            # getting optype
            optype = get_flash_optype(self.device, propertiesMap, filepath)
                
            self.push_message(str("optype: " + optype +", Offset: " + str(offset)), FP_TRACE_LEVEL_INFO)   
            ...
            self.push_message(str(e), FP_TRACE_LEVEL_FATAL)
            return None
        ...
        if propertiesMap['CustomFlashEnable'] == True:
            ...
            try:
                if (self.device == "am243x"):
                    ...
                    if (propertiesMap['FlashPartType'] == "SIP"):
                        flasher = default_flashwriter[self.device]["SIP"]
                    else:
                        flasher = default_flashwriter[self.device]["CC"]
                elif (self.device == "am261x"):
                    if (propertiesMap['FlashBoardType'] == "SOM"):
                        flasher = default_flashwriter[self.device]["SOM"]
                    else:
                        flasher = default_flashwriter[self.device]["LP"]
