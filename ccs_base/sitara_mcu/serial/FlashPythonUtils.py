...
devices = ["am243", "am263p", "am263", "am261", "am273"]
...
default_flashwriter = {
    ...
    "am263px": {"CC": "am263px.release.tiimage", "SIP": "am263px_sip.release.tiimage"},
    "am261x": {"SOM": "am261x_som.release.tiimage", "LP": "am261x_lp.release.tiimage"},
...
def get_supported_file_types(device):
    supported_file_types = {}
    
    if device in ["am263x", "am263px", "am261x"]:
        supported_file_types = {'tiimage', 'mcelf', 'mcelf.hs', 'mcelf.hs_fs', 'mcelf_xip', 'hsmimage.bin'}
    else:
        supported_file_types = {'tiimage', 'appimage', 'mcelf', 'appimage.hs_fs', 'appimage.hs', 'appimage_xip',  'mcelf.hs', 'mcelf_xip', 'hsmimage.bin'}
...
file_offset_type = {
    ...
    "mcelf.hs": "app_offset",
    "mcelf.hs_fs": "app_offset",
    "appimage_xip": "xip_offset",
    "mcelf_xip": "xip_offset",
    "hsmimage.bin": "hsmrtimg_offset"
}
...
def create_temp_file(filename, offset, optype):
    ...
    offset_val = rsv_word
    if(optype not in ("flash-xip", "flashverify-xip", "flash-mcelf-xip", "flashverify-mcelf-xip", "flash-phy-tuning-data")):
        offset_val = offset
...
def get_file_offset(filename, device):
    ...
    supported_file_types = get_supported_file_types(device)
    filetype = [
...
def get_flash_optype(device, properties, filepath):
        optype = "flash"
        flash_sector_devices = ["am261x", "am263px"]
        
        if device in ["am263x", "am263px", "am261x"]:
            if properties['FlashOperationType'] == 'Flash Verify':
                optype = "flashverify"
            elif device in flash_sector_devices:
                optype = "flash-sector-write"
            
        # handle xip files     
        if filepath.endswith("mcelf_xip"):
            if optype in ["flash", "flash-sector-write"]:
                optype = "flash-mcelf-xip"
            elif optype == "flashverify":
                optype = "flashverify-mcelf-xip"
...
def get_device_name(partnum):
    if (partnum.lower().startswith("am243")):
        ...
        return "am263x"
    elif (partnum.lower().startswith("am261")):
        return "am261x"
